...
class SubscriptionActivity : BaseVbActivity<ActivitySubscriptionBinding>() {
    ...
    private fun initBottomNavigation() {
        // 底部导航已改为自绘 LinearLayout(见 include_bottom_navigation.xml),
        // 订阅页只把"订阅"高亮,其余 tab 点击后 finish() 回到主页对应页。
        val nav = mBinding.bottomNavRoot
        val selected = androidx.core.content.ContextCompat.getColor(this, R.color.bili_pink)
        val normal = androidx.core.content.ContextCompat.getColor(this, R.color.bili_text_tertiary)

        nav.ivHome.setColorFilter(normal)
        nav.tvHome.setTextColor(normal)
        nav.ivLive.setColorFilter(normal)
        nav.tvLive.setTextColor(normal)
        nav.ivSubscribe.setColorFilter(selected)
        nav.tvSubscribe.setTextColor(selected)
        nav.ivMy.setColorFilter(normal)
        nav.tvMy.setTextColor(normal)

        nav.tabSubscribe.setOnClickListener { /* 当前页,不做处理 */ }
        nav.tabHome.setOnClickListener {
            destinationAfterFinish = R.id.navigation_home
            finish()
        }
        nav.tabLive.setOnClickListener {
            destinationAfterFinish = R.id.navigation_live
            finish()
        }
        nav.tabMy.setOnClickListener {
            destinationAfterFinish = R.id.navigation_dashboard
            finish()
        }
    }
