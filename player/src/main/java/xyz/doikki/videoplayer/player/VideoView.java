...
public class VideoView<P extends AbstractPlayer> extends FrameLayout
    ...
    public void startFullScreen() {
        ...
        //注意:必须等 hideSysBar 之后再 addView,否则 measure 时机拿到的是带状态栏的旧尺寸,
        //fitCenter 居中会偏上(小米14/API34+ 全面屏沉浸下尤为明显);
        //显式 Gravity.CENTER + 重新 requestLayout 保证用真实全屏尺寸重新居中。
        LayoutParams params = new LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT,
                Gravity.CENTER);
        decorView.addView(mPlayerContainer, params);
        Log.d("TVBoxDiag", "VideoView.startFullScreen decorView=" + decorView.getWidth() + "x" + decorView.getHeight());
    ...
    public void startTinyScreen() {
        ...
        // 小窗用 Gravity.BOTTOM|END 贴 contentView 右下角。
        // 注意:Gravity.BOTTOM 下 topMargin 无约束意义(会被忽略),不要加,否则在部分 ROM
        // 上会把小窗顶到状态栏区域导致顶部被裁。
        // 只避让底部导航栏/手势条,避免小窗压在系统栏上。
        int navBarH = PlayerUtils.getNavigationBarHeight(getContext());

        LayoutParams params = new LayoutParams(width, height);
        params.gravity = Gravity.BOTTOM | Gravity.END;
        Log.d("TVBoxDiag", "VideoView.startTinyScreen contentView=" + contentView.getWidth() + "x" + contentView.getHeight()
                + " finalWH=" + width + "x" + height + " nav=" + navBarH);
        params.bottomMargin = Math.max(navBarH, 0);
        params.rightMargin = Math.max(navBarH, 0);
    ...
    public void onVideoSizeChanged(int videoWidth, int videoHeight) {
        ...
        Log.d("TVBoxDiag", "VideoView.onVideoSizeChanged video=" + videoWidth + "x" + videoHeight
                + " scale=" + mCurrentScreenScaleType + " isFull=" + mIsFullScreen + " isTiny=" + mIsTinyScreen);
    ...
    /**
     * 获取当前 video 缩放模式。
     * 进入 PiP 浮窗前需要保存这个值,退出 PiP 时恢复(因为 PiP 期间会临时改成
     * CENTER_CROP 让视频撑满浮窗,跟用户选择的"默认/16:9/4:3"不一致)。
     */
    public int getCurrentScreenScaleType() {
