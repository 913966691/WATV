...
public class JarLoader {
     ...
     * @param cache
     */
    /** load() 失败时返回的字符串,提示真实异常;null 表示加载成功。 */
    ...
    private boolean loadClassLoader(String jar, String key) {
        ...
        try {
            ...
            if (!cacheDir.exists())
                cacheDir.mkdirs();
            // Android 14 (API 34) 兼容性:dex 优化目录也最好只读。
            // 目录本身可写会引发后续 SecurityException。setReadOnly 对目录返回 true 表示成功。
            try {
            ...
            if (!jarFile.exists()) {
                lastLoadError = "jar文件不存在";
            } else if (jarFile.length() < 100) {
                lastLoadError = "jar文件过小(" + jarFile.length() + "字节),可能下载不完整";
            } else {
                ...
                do {
                    try {
                        ...
                        if (classInit != null) {
                            ...
                            android.util.Log.d("JarLoader", "自定义爬虫代码加载成功!");
                            success = true;
                            try {
        ...
        if (!success && lastLoadError == null) {
            if (lastError != null) {
                String name = lastError.getClass().getSimpleName();
                String msg = lastError.getMessage();
                lastLoadError = (msg != null && !msg.isEmpty()) ? (name + ": " + msg) : name;
            } else {
                lastLoadError = "未找到 com.github.catvod.spider.Init 类(jar非合法csp/包名不一致)";
            }
        }
        return success;
    }
